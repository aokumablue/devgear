...
# build_onnx_if_missing <repo_root> <model_target_dir> <quant> [revision]
#   model_target_dir に model.onnx が存在しない場合のみビルドする（冪等）。
#   repo_root/.venv-modelbuild をビルド用 venv として使用する。
#
# build_onnx_always <repo_root> <model_target_dir> <quant> [revision]
#   冪等チェックなしで必ずビルドする。
build_onnx_impl() {
  local repo_root="$1"
  ...
  local build_venv="${repo_root}/.venv-modelbuild"
  local build_python="${build_venv}/bin/python3"
  local build_reqs="${repo_root}/scripts/requirements-build.txt"
...
build_onnx_if_missing() {
  ...
  build_onnx_impl "${repo_root}" "${model_target}" "${quant}" "${revision}"
