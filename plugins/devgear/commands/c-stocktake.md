...
RESULTS_JSON="${DEVGEAR_DIR:-$HOME/.devgear}/stocktake/results.json"
```
...
以下を実行してスキルファイルを列挙し frontmatter・mtime を収集する:

```bash
devgear_run devgear.skills.stocktake.cli scan
```
...
`~/.devgear/stocktake/results.json`:
...
`evaluated_at` は実際のUTC時刻。`T00:00:00Z` の近似値は禁止。
